...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"delta0_sfdm",pba->has_sfdm);
      class_store_columntitle(titles,"delta1_sfdm",pba->has_sfdm);
...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    class_define_index(ppv->index_pt_delta0_sfdm,pba->has_sfdm,index_pt,1); /* scalar field dark perturbation auxiliary variable delta_0 */
    class_define_index(ppv->index_pt_delta1_sfdm,pba->has_sfdm,index_pt,1); /* scalar field dark perturbation auxiliary variable delta_1 */
    ...
    */
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      if (pba->has_sfdm == _TRUE_) {
        // aquí realmente delta_sfdm = 0. ya que es delta_0=delta_1=0 en las C.I. Aunque deberia ser la expresion rho_phi*(delta_{rho \phi}), donde delta_{rho \phi} es una función de delta_0 y delta_1
        delta_sfdm = 0.;
        theta_sfdm = 0.;
      }
  ...
  if (_scalars_) {
    ...
    if (pba->has_sfdm == _TRUE_) {
      ...
      //if (ppt->gauge == synchronous){
        ...
        /* Add SFDM to the total perturbation sources */
	  ...
	  /* Add SFDM to the total matter density contrast */
	  if (ppt->has_source_delta_m == _TRUE_){
	    delta_rho_m += delta_rho_sfdm;
	    rho_m += rho_sfdm;
	  }
	  /* Add SFDM to the total matter velocity divergence */
    if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)){
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_delta_sfdm == _TRUE_ || ppt->has_source_theta_sfdm == _TRUE_) {
      ...
      if (ppt->has_source_delta_sfdm == _TRUE_) {
        delta_sfdm =  delta0;    
        _set_source_(ppt->index_tp_delta_sfdm) = delta_sfdm + 3.*a_prime_over_a*(1. - cos_th)*theta_over_k2; // N-body gauge correction
      }
      if (ppt->has_source_theta_sfdm == _TRUE_) {
        double rho_plus_p_sfdm = rho_sfdm + p_sfdm;
        double y1 = pvecback[pba->index_bg_y1_sfdm];
        rho_plus_p_theta_sfdm = k*k*rho_sfdm*(delta1*(1.-cos_th)-delta0*sin_th)/ (a_prime_over_a*y1);
  ...
  if (_scalars_) {
    ...
    if (pba->has_sfdm == _TRUE_){
      ...
      double rho_plus_p_sfdm = rho_sfdm + p_sfdm;
        ...
        rho_plus_p_theta_sfdm = k*k*rho_sfdm*(delta1_sfdm*(1.-cos_th)-delta0_sfdm*sin_th )/ (a*H*y1);
    ...
    if ((ppt->gauge == synchronous) && (ppt->get_perturbations_in_current_gauge == _FALSE_)) {
      ...
      if (pba->has_sfdm == _TRUE_) {
          /* Gauge transformation for physical SFDM density contrast and velocity only */
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_sfdm == _TRUE_) {
      ...
      dy[pv->index_pt_delta0_sfdm] = -a_prime_over_a*((3.*sin_th+k2_over_kJ2*one_min_cos)*delta1_sfdm -k2_over_kJ2*sin_th*delta0_sfdm)
                                                         -metric_continuity*one_min_cos; /* metric_continuity equals h'/2 in synchronous gauge */
      dy[pv->index_pt_delta1_sfdm] = -a_prime_over_a*((3.*cos_th+k2_over_kJ2*sin_th)*delta1_sfdm -k2_over_kJ2*one_plus_cos*delta0_sfdm)
                                                        -metric_continuity*sin_th; /* metric_continuity equals h'/2 in synchronous gauge */                                                
    }
    /** - ---> scalar field (scf) */
