...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  if (pfzw->required_computation_stage >= cs_background){
    ...
    class_call_except(background_init(&pr,&ba), ba.error_message, errmsg, background_free(&ba);thermodynamics_free_input(&th);perturbations_free_input(&pt););
  }
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  if (pba->Omega0_sfdm != 0.){
    ...
    class_test(flag1 == _FALSE_,
                errmsg,
                "You requested Omega_sfdm != 0, but did not provide 'sfdm_parameters'.");

    class_test(pba->sfdm_parameters_size < 2,
                errmsg,
                "The parameter 'sfdm_parameters' must contain at least [log10(m/eV), tuning].");

    class_test(pba->sfdm_tuning_index < 0,
                errmsg,
                "Tuning index 'sfdm_tuning_index' must be non-negative.");
    ...
    class_call(parser_read_string(pfc,
      ...
      /*double Omega_rad0 = pba->Omega0_g + pba->Omega0_ur; Distribución valida para 3 netrinos rel, o dos rel y uno masivo*/
      double Omega_rad0 = pba->Omega0_g*(1. + 3.044*7./8.*pow(4./11.,4./3.));
      /* m_phi in H units, m_phi = exp(pba->sfdm_parameters[0] * log(10.0)) */
      double m_over_H0 = 15.64 * pow(10., pba->sfdm_parameters[0]) / pba->H0;
      ...
      alpha_sfdm = pba->sfdm_parameters[pba->sfdm_tuning_index] + 0.5 * log(pba->Omega0_sfdm * 1.e-14 / (aosc3 * Omega_rad0));
      ...
      pba->alpha_ini_sfdm = alpha_sfdm;
        ...
        /* Test */
        class_test(pba->sfdm_parameters_size < 4,
                  errmsg,
                  "Since you are not using attractor IC, you must specify SFDM variables as the last two entries in sfdm_parameters. See sfdm.ini for more details.");
            pba->y1_ini_sfdm = y1_sfdm;
            /* Complete set of parameters */
...
int input_default_params(struct background *pba,
  ...
  /* ** ADDITIONAL SPECIES ** */
  /** 8) SFDM defaults contributions */
  pba->Omega0_sfdm = 0.; 
  pba->attractor_ic_sfdm = _TRUE_;
  pba->sfdm_parameters = NULL;
  pba->sfdm_parameters_size = 0;
  pba->sfdm_tuning_index = 1;
  ...
  pba->Omega0_fld = 0.;
  pba->Omega0_scf = 0.;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr - pba->Omega0_idr -pba->Omega0_idm -pba->Omega0_sfdm;
  /** 8.a) Omega fluid */
  /** 8.a.1) PPF approximation */
  pba->use_ppf = _TRUE_;
