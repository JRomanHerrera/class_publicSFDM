...
int background_functions(
  ...
  if (pba->has_sfdm == _TRUE_) {
    ...
    /*alpha in the code corresponds to beta in arXiv:2307.05600, with Omega_sfdm = exp(2*alpha).*/
    ...
    rho_sfdm = exp2a*rho_tot/(1.-exp2a); // energy of the sfdm.
    p_sfdm   = -cos_th * rho_sfdm; // pressure of the sfdm p_sfdm=w_sfdm *rho_sfdm
  ...
  */
  ...
  if(pba->has_sfdm == _TRUE_){
    ...
    class_test(!isfinite(pvecback_integration[pba->index_bi_theta_sfdm]) ||
           !isfinite(pvecback_integration[pba->index_bi_y1_sfdm]) ||
           !isfinite(pvecback_integration[pba->index_bi_alpha_sfdm]),
           pba->error_message,
           "Invalid SFDM initial conditions: "
           "initial theta_sfdm = %e, y1_sfdm = %e ,alpha_sfdm = %e.",
           pvecback_integration[pba->index_bi_theta_sfdm],
           pvecback_integration[pba->index_bi_y1_sfdm],
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"Omega_sfdm",pba->has_sfdm);
